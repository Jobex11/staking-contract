...
contract RBStaking {
    ...
    // Function to add multiple eligible wallet addresses
    function addWallets(
        ...
        for (uint256 i = 0; i < _wallets.length; i++) {
            ...
            if (category == 1) {
                soldBeforeJune17.push(wallet);
            } else if (category == 2) {
                purchasedBeforeAug1HeldOrSoldAfterJune17.push(wallet);
            } else if (category == 3) {
                purchasedAfterJuly22.push(wallet);
            }
            emit WalletAdded(wallet, category);
        }
    }
    ...
    function withdraw() external {
        ...
        require(userStake.staked, "No active stake");
        require(
            block.timestamp >= userStake.startTime + 360 minutes,
...
// for 360 minutes ..
//0x7B86ea7f3902b2c04b7c7EaDde5b5d37A60bC014
